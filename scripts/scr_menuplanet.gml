        ...
        for(var i=0; i<count; i+=1)
        {
            if str2[i] != 0 && str2[i] != "none" {
                ...
                if device_mouse_check_button_released(0,mb_left)  && point_distance(xxmouse,yymouse,room_width-(room_width/12),100+(70*listNumberShipTravel) ) < 30  {
                    ...
                    //if global.travel > 0 && global.travel < 500{
                        with(obj_highscore){
                            var hiscore_map, i, str;
                            hiscore_map = ds_map_create();
...
point_distance(xxmouse,yymouse,75,(room_height/20)) < 70 {
...
if device_mouse_check_button_released(0, mb_left)  {scr_save();game_end();}
...
if device_mouse_check_button_pressed(0, mb_left) && 
point_distance(xxmouse,yymouse,75,room_height-(room_height/10)) < 30 {
...
if planetnowid = 0 && ds_grid_get(obj_planetary.planetary,32,global.to) != global.idfixe && global.idguild != ds_grid_get(planetary,242,planetnowid){
...
}else if (real(global.to) != planetnowid and ds_grid_get(planetary,32,global.to) == global.idfixe) or (global.to != planetnowid and global.idguild = real(ds_grid_get(planetary,242,planetnowid))){
draw_sprite_ext(spr_stats,6,xxmenulerp+xdisthealth-xplus+100,2750+ytext-(roll*4),1,1,0,colorrect9,1)
...
draw_set_color(c_white);
draw_text_outline(xxmenulerp+xdisthealth-xplus,300,"Fleet trip",planetarycolor2,room_width,fa_bottom,20,true)
...
if ds_grid_get(planetary,32,planetnowid) = global.idfixe {
    ...
    draw_set_color(planetarycolor2);//show_debug_message(techArray[i])
    ...
    if techArray[i+1] > 0 {texticon1 = " Level 1 upgraded"}else /*if scoresciencevalid < levelrequired+1 */{texticon1 = "technology point "+string(scoresciencevalid)+" / "+string(levelrequired+(1*(i+1)))}
    if scoresciencevalid <= levelrequired+(1*(i+1)) {coloricon1 = c_red} else if techArray[i+1] = 0 && scoresciencevalid >= levelrequired+(1*(i+1)) {coloricon1 = c_green
    if device_mouse_check_button(0, mb_left) {if techArray[i+1] = 0 {techArray[i+1] = 1}}}
    }
    ...
    if techArray[i+1] < 1 or scoresciencevalid < levelrequired+(2*(i+1)) {coloricon2 = c_red}else if techArray[i+1] = 1 && scoresciencevalid >= levelrequired+(2*(i+1)){coloricon2 = c_green
    if device_mouse_check_button(0, mb_left) {if techArray[i+1] >= 1 {techArray[i+1] = 2}}}
    ...
    if techArray[i+1] < 2 or scoresciencevalid < levelrequired+(3*(i+1)) {coloricon3 = c_red}else if techArray[i+1] = 2 && scoresciencevalid >= levelrequired+(3*(i+1)){coloricon3 = c_green
    if device_mouse_check_button(0, mb_left) {if techArray[i+1] >= 2 {techArray[i+1] = 3}}}
...
draw_rectangle(widthor0+xxmenulerp,275,room_width+xxmenulerp,340,false);
draw_set_color(c_white);
draw_text_outline(xxmenulerp+xdisthealth-xplus,300,'Galactic technology point '+ string(floor(scoresciencevalid)),planetarycolor2,room_width,fa_bottom,20,true)
...
ds_grid_set(planetary,51,planetnowid,floor(ds_grid_get(planetary,50,planetnowid)/(1+(ds_grid_get(planetary,50,0)))))
...
draw_text_outline(xxmenulerp+xdisthealth-xplus,500+ytext,"Id galactic: "+string(planetnowid),planetarycolor2,room_width,fa_bottom,20,false)//ds_grid_add(planetary,9,yygrid,26)//Velocity 26 km/s
...
draw_set_color(planetarycolor2)
draw_text_outline(xxmenulerp+xdisthealth-xplus+70,370+ytext,"Resources : "+string_add_delimiter(floor(ds_grid_get(planetary,35,planetnowid))),planetarycolor2,room_width,fa_bottom,20,false)//ds_grid_add(planetary,2,yygrid,150000000)//distance soleil
...
draw_sprite_ext(spr_quitplanet,1,room_width-(room_width/4)+xxmenulerp,(room_height/6),1,1,0,planetarycolor2,1)
draw_set_halign(fa_right);
