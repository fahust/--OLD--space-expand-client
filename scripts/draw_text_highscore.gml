...
var height = 25;
var max_la = 30; // minimum width of the first column (the nr.)
var max_lb = 300; // minimum width of the name column
var max_lc = 300; // minimum width of the score column
var max_ld = 10; // minimum width of the score column
var max_le = 10; // minimum width of the score column
var max_lf = 10; // minimum width of the score column
var max_lg = 10; // minimum width of the score column
var max_lh = 10; // minimum width of the score column
var max_li = 10; // minimum width of the score column
var max_lj = 10; // minimum width of the score column
var max_lk = 10; // minimum width of the score column
}
...
for(var i=0; i<count+1; i+=1)
{
    ...
    /*show_message(string(txt3b[i]))
    show_message(real(txt3c[i]))
    show_message(real(txt3d[i]))*/
...
for(var i=0; i<count+1; i+=1)
{
    ...
    with(obj_highscore){instance_destroy()}
    ...
    messagefinal = string(txt3h[i])+" ("+string(date_datetime_string(real(txt3e[i])/100000))+")  :  "+string(txt3c[i])
    draw_set_valign(fa_top);
    if obj_planetary.portrait = 0 {
...
//if txt3c[i] = "str" {//TITRE
    ...
    if obj_planetary.menuplanet3 = 3 && obj_planetary.guildchannel = 1{draw_text(xx+10,yy,"Guild Communication");}
//}
    draw_text_highscore2(0, 50, "line", "Name", "Score", string(text2), player_name, c_green,"RESS","SHIP","SCIENCE","DEF","guildname","guildid","guildinvit","grade");
}
draw_set_valign(fa_top);
...
if point_distance(xxmouse,yymouse,xxmouse,room_height-(room_height/3.4)+0) < 15 {colorrect11 = c_white
if device_mouse_check_button(0, mb_left) {
if guildchannel = 1 {guildchannel = 0}else{guildchannel = 1}
if instance_exists(obj_highscore){with(obj_highscore){instance_destroy();}}}}else{colorrect11 = planetarycolor2}
draw_set_colour(planetarycolor);
if guildchannel = 1 {
...
draw_rectangle_colour(room_width-(room_width/40),(room_height/15)+(roll/1),room_width,(room_height/15)+(roll/1)+30,c_white,c_white,c_white,c_white,false)
   if device_mouse_check_button(0, mb_left) && 
point_distance(xxmouse,yymouse,room_width,yymouse) < 15 && yymouse < room_height-(room_height/2.1)-40 {
...
if menuplanet3 = 2 {
if point_distance(xxmouse,yymouse,xxmouse,room_height-(room_height/2.1)+10) < 15 {colorrect10 = c_white
   if device_mouse_check_button(0, mb_left) {
...
if point_distance(xxmouse,yymouse,xxmouse,room_height-(room_height/2.4)-20) < 15 {colorrect12 = c_white
...
if point_distance(xxmouse,yymouse,xxmouse,room_height-(room_height/2.8)-20) < 15 { colorrect13 = c_white
if device_mouse_check_button_pressed(0, mb_left) {
showInputText("Change guild name", "Change", "Cancel")
keyboard_string = ""
keyboardcheck2 = 0
global.idguild = global.idfixe
...
for(var i=0; i<count+1; i+=1)
{
    ...
    draw_set_valign(fa_bottom);
    draw_set_colour(obj_planetary.planetarycolor2);
    if obj_planetary.portrait = 1 {draw_rectangle(0,yy+height+44-(obj_planetary.roll*10),room_width,yy+height+46-(obj_planetary.roll*10),false);}else{
    draw_rectangle(0,yy+height+49-(obj_planetary.roll*10),room_width,yy+height+51-(obj_planetary.roll*10),false);}
    //draw_rectangle(0,yy+height+49-(obj_planetary.roll*10),room_width,yy+height+51-(obj_planetary.roll*10),false);
    if i <= 0 {
    draw_set_colour(obj_planetary.planetarycolor);}else{
    draw_set_colour(obj_planetary.planetarycolor2);}
    
    if i = 0 {draw_text(xx+max_la+5,yy+height,txt3a[i]);}else{
    draw_text(xx+max_la+5,yy+height-(obj_planetary.roll*10),txt3a[i]);
    }
    ...
    if i = 0 {draw_text(xx+max_la+30,yy+height,txt3b[i]);}else{
    draw_text(xx+max_la+30,yy+height-(obj_planetary.roll*10),txt3b[i]);}
    
    
    if txt3c[i] = "score" {draw_sprite_ext(spr_highscore,0,xx+max_la+30+max_lb+30,yy+height-30,0.5,0.5,0,obj_planetary.planetarycolor,1);}else{
    draw_text(xx+max_la+30+max_lb+10,yy+height-(obj_planetary.roll*10),txt3c[i]);}
    //if i = 0 {draw_text(xx+max_la+30+max_lb+10,yy+height,txt3c[i]);}else{
    //draw_text(xx+max_la+30+max_lb+10,yy+height-(obj_planetary.roll*10),txt3c[i]);}
    if obj_planetary.portrait = 0 {
    if txt3d[i] = "RESS" {draw_sprite_ext(spr_stats,0,xx+max_la+30+max_lb+max_lc+30,yy+height-30,0.3,0.3,0,obj_planetary.planetarycolor,1)}else{
    if real(txt3d[i]) > 0 {if real(txt3d[i])/1000000 > 1 {
    draw_text(xx+max_la+30+max_lb+max_lc+10,yy+height-(obj_planetary.roll*10),"> "+string(round(real(txt3d[i])/1000000))+" M");}else{
    draw_text(xx+max_la+30+max_lb+max_lc+10,yy+height-(obj_planetary.roll*10),"< 1 M");}}}
    
    if txt3e[i] = "SHIP" {draw_sprite_ext(spr_stats,1,xx+max_la+30+max_lb+max_lc+max_ld+30,yy+height-30,0.3,0.3,0,obj_planetary.planetarycolor,1)}else{
    draw_text(xx+max_la+30+max_lb+max_lc+max_ld+10,yy+height-(obj_planetary.roll*10),txt3e[i]);}
    
    if txt3f[i] = "SCIENCE" {draw_sprite_ext(spr_stats,2,xx+max_la+30+max_lb+max_lc+max_ld+max_le+30,yy+height-30,0.3,0.3,0,obj_planetary.planetarycolor,1)}else{
    draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+10,yy+height-(obj_planetary.roll*10),txt3f[i]);}
    
    if txt3g[i] = "DEF" {draw_sprite_ext(spr_stats,3,xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+30,yy+height-30,0.3,0.3,0,obj_planetary.planetarycolor,1)}else{
    draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+10,yy+height-(obj_planetary.roll*10),txt3g[i]);}//
    
    if i = 0 {draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+max_lh+10,yy+height,txt3h[i]);}else{
    draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+max_lh+10,yy+height-(obj_planetary.roll*10),txt3h[i]);}
    //draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+max_lh+max_li+10,yy+height,txt3i[i]);
    //draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+max_lh+max_li+max_lk+10,yy+height,txt3k[i]);
    //draw_text(xx+max_la+30+max_lb+max_lc+max_ld+max_le+max_lf+max_lh+max_li+max_lj+10,yy+height,txt3j[i]);
    }
